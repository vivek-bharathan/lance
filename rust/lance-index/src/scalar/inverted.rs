...
/// statistics rather than per-segment statistics.
///
/// Public so distributed-FTS coordinators that aggregate per-host stats
/// out-of-band can reuse the same arithmetic instead of reimplementing it.
/// The function takes ownership of computing the union of fuzzy-expanded
/// terms when `params.fuzziness` is set.
